...
5. The leader returns a compact structured result: winner ID, winner score, runner-up score, and a reason-code enum.
6. Validators independently repeat the full evidence review.
7. Consensus requires the same winner, bounded agreement on both numeric scores, and exact agreement on the reason code.
8. The human-readable rationale is composed deterministically from those agreed fields, so displayed settlement facts do not depend on unverified leader prose.
9. Deterministic contract state records the accepted result.
